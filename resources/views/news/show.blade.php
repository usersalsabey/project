...
<style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .logo-text span:first-child { font-size:15px; font-weight:800; color:#fff; line-height:1.2; }
    ...
    nav ul li a.active { color:#fff; background:var(--accent); }
    ...
    .dark-toggle:hover { background:rgba(255,255,255,0.14); color:var(--gold-lt); }
    .dark-toggle .icon-moon { display:block; } .dark-toggle .icon-sun { display:none; }
    html.dark .dark-toggle .icon-moon { display:none; } html.dark .dark-toggle .icon-sun { display:block; }
    ...
    @media (max-width:900px) { .page-wrap{grid-template-columns:1fr} .hero-title{font-size:26px} .hero-inner{padding:0 28px 32px} header{padding:0 24px} .footer-location{padding:18px 28px} .footer-main{padding:44px 28px 28px} .footer-grid{grid-template-columns:1fr 1fr} }
    @media (max-width:640px) { .page-wrap{padding:24px 20px 60px} .article-content{padding:24px 20px 28px} .article-footer{padding:16px 20px} .hero{height:320px} .hero-title{font-size:22px} .gallery-main{height:240px} nav ul li a{padding:7px 10px;font-size:12px} .footer-grid{grid-template-columns:1fr} .footer-bottom{flex-direction:column;gap:8px;text-align:center} }
</style>
@endpush
...
<section class="hero">
    <div class="hero-bg">
        @if(!empty($news['images']) && count($news['images']) > 0)
            <img src="{{ asset($news['images'][0]) }}" alt="{{ $news['title'] }}">
        @else
    ...
    <div class="hero-inner">
        ...
        <div class="hero-badge">
            <span class="cat-badge">{{ match($news['category']) { 'lalu_lintas'=>'🚦 Lalu Lintas','pelayanan'=>'🆔 Pelayanan','sosial'=>'🤝 Sosial','kriminal'=>'🔒 Kriminal',default=>'📰 Umum' } }}</span>
        </div>
    ...
    <article class="article-body">
        ...
        <div class="gallery">
            <div class="gallery-main">
                <img src="{{ asset($news['images'][0]) }}" alt="{{ $news['title'] }}" id="mainImg">
            </div>
            @if(count($news['images']) > 1)
            <div class="gallery-thumbs">
                @foreach($news['images'] as $i => $img)
                <div class="thumb {{ $i === 0 ? 'active' : '' }}" onclick="switchImg('{{ asset($img) }}', this)">
                    <img src="{{ asset($img) }}" alt="Foto {{ $i+1 }}">
                </div>
                @endforeach
            </div>
            @endif
        ...
        @else
        <div class="gallery-placeholder cat-{{ $news['category'] }}">{{ $news['icon'] ?? '📰' }}</div>
        @endif
        ...
        <div class="article-footer">
            ...
            <div class="share-btns">
                ...
                <button class="share-btn copy" id="copyBtn" onclick="copyLink()">🔗 Salin Link</button>
                <a class="share-btn wa" id="waShareBtn" href="#" target="_blank" rel="noopener">💬 WhatsApp</a>
            </div>
        </div>
    </article>
            ...
            <a href="{{ route('news.show', $item['slug']) }}" class="related-item">
                <div class="related-thumb">
                    @if(!empty($item['images']) && count($item['images']) > 0)
                        <img src="{{ asset($item['images'][0]) }}" alt="{{ $item['title'] }}">
                    @else
                        {{ $item['icon'] ?? '📰' }}
                    @endif
                </div>
                <div class="related-info">
                    <div class="related-cat">{{ match($item['category']) { 'lalu_lintas'=>'Lalu Lintas','pelayanan'=>'Pelayanan','sosial'=>'Sosial','kriminal'=>'Kriminal',default=>'Umum' } }}</div>
...
// DARK MODE
(function () {
    const html = document.documentElement, btn = document.getElementById('darkToggle'), KEY = 'theme';
...
// SET WHATSAPP SHARE URL — pakai window.location supaya selalu URL yang benar
(function () {
    const title = @json($news['title']);
...
(function () {
    ...
    const WMO = { 0:{e:'☀️',d:'Cerah'},1:{e:'🌤️',d:'Umumnya Cerah'},2:{e:'⛅',d:'Berawan Sebagian'},3:{e:'☁️',d:'Berawan'},45:{e:'🌫️',d:'Berkabut'},61:{e:'🌧️',d:'Hujan Ringan'},63:{e:'🌧️',d:'Hujan Sedang'},65:{e:'🌧️',d:'Hujan Lebat'},80:{e:'🌦️',d:'Hujan Lokal'},95:{e:'⛈️',d:'Hujan Petir'} };
    const HARI = ['Min','Sen','Sel','Rab','Kam','Jum','Sab'];
    function wmo(c) { return WMO[c] || {e:'🌡️',d:'—'}; }
    const API = 'https://api.open-meteo.com/v1/forecast?latitude='+LAT+'&longitude='+LON+'&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone='+TZ+'&forecast_days=7';
    async function load() {
        try {
    ...
    document.getElementById('wxRefresh').addEventListener('click', load); load();
})();
</script>
@endpush